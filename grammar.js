  ...
  rules: {
    ...
    drop_function_item: ($) =>
      seq(
        optional($.if_exists),
        $.identifier,
        optional(seq("(", commaSep(choice($.var_declaration, $._type)), ")"))
      ),
    ...
    insert_statement: ($) =>
      seq(
        ...
        $.identifier,
        optional($.as),
        optional(alias($._list_of_identifiers, $.columns)),
        $.insert_items,
        optional($.insert_conflict),
        optional($.returning),
        optional($.into)
      ),
    ...
    create_table_item: ($) => choice($.table_column_item, $.table_constraint),
    ...
    create_schema_statement: ($) =>
      seq(
        ...
        kw("schema"),
        optional($.if_not_exists),
        choice(seq($.identifier, optional($.schema_role)), $.schema_role)
      ),
    ...
    psql_statement: ($) => seq("\\", repeat1($.identifier), /[\n\r]/),
    ...
    create_trigger_statement: ($) =>
      seq(
        ...
        kw("on"),
        $.identifier,
        optional($.trigger_scope),
        optional($.trigger_cond),
        $.trigger_exec
      ),

    trigger_when: ($) => choice(kw("before"), kw("after"), kw("instead of")),

    trigger_event: ($) =>
      separated1(
        kw("or"),
        choice(kw("insert"), kw("update"), kw("delete"), kw("truncate"))
      ),
    ...
    trigger_cond: ($) => seq(kw("when"), "(", $._value_expression, ")"),
    ...
    with_query_item: ($) =>
      seq(
        ...
        kw("as"),
        optional(
          choice(kw("materialized"), seq(kw("not"), kw("materialized")))
        ),
        "(",
        $._with_query_statement,
        ")"
    ...
    function_run_as: ($) =>
      seq(kw("security"), choice(kw("invoker"), kw("definer"))),
    ...
    function_volatility: ($) =>
      choice(kw("immutable"), kw("stable"), kw("volatile")),

    block: ($) =>
      seq($.dollar_quote, repeat($.declarations), $.body, $.dollar_quote),

    body: ($) =>
      seq(kw("begin"), repeat($._plpgsql_statement), kw("end"), optional(";")),

    dollar_quote: ($) => seq("$", optional($.identifier), "$"),

    declarations: ($) => seq(kw("declare"), repeat($.var_definition)),

    var_definition: ($) =>
      seq($.var_declaration, optional(seq(":=", $._value_expression)), ";"),

    function_signature: ($) => seq($.identifier, $.function_parameters),
    ...
    function_parameters: ($) =>
      seq(
        ...
        optional(
          seq(kw("default"), field("default_value", $._value_expression))
        ),
        ")"
      ),
